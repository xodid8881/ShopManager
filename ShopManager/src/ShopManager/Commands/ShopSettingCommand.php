...
final class ShopSettingCommand extends Command{
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) : bool{
        ...
        if(!isset($args[0])){
            ...
            $sender->sendMessage(ShopManager::TAG . "/상점설정 삭제 ( 상점이름 ) < 상점을 삭제합니다. >");
            ...
            $sender->sendMessage(ShopManager::TAG . "/상점설정 프리셋설정  ( 상점이름 ) < 상점 프리셋 물품을 설정합니다. >");
            $sender->sendMessage(ShopManager::TAG . "/상점설정 목록 < 상점목록을 확인합니다. >");
            return true;
        }
        switch($args [0]){
            case "목록" :
                foreach($this->api->shopdb ["프리셋"] as $shopname => $v){
                    if (isset($this->api->shopdb ["프리셋"] [$shopname])){
                        $sender->sendMessage(ShopManager::TAG . $shopname);
                    }
                }
                $sender->sendMessage(ShopManager::TAG . "상점들이 존재합니다.");
                return true;
            case "프리셋설정" :
                if(isset($args[1])){
                    if(!isset($this->api->shopdb [$args[1]])){
                        $sender->sendMessage(ShopManager::TAG . "해당 이름으로 상점이 없습니다.");
                        return true;
                    }
                    $item = $sender->getInventory()->getItemInHand();
                    if($item == VanillaItems::AIR()){
                        $sender->sendMessage(ShopManager::TAG . "손에 프리셋에 보여줄 아이템을 들고 진행해주세요.");
                        return true;
                    }
                    $item = $sender->getInventory()->getItemInHand();
                    ...
                    $sender->sendMessage(ShopManager::TAG . $args[1] . "상점 프리셋 물품을 변경했습니다.");
                    return true;
                }else{
                    $sender->sendMessage(ShopManager::TAG . "/상점설정 프리셋설정  ( 상점이름 ) < 상점 프리셋 물품을 설정합니다. >");
                    return true;
            ...
            case "삭제" :
                if(isset($args[1])){
                    ...
                    unset ($this->api->shopdb [$args[1]]);
                    unset ($this->api->shopdb ["프리셋"] [$args[1]]);
                    unset ($this->api->shopdb [$args[1]] ["물품"]);
                    $sender->sendMessage(ShopManager::TAG . $args[1] . "상점이 삭제되었습니다.");
                    return true;
                }else{
                    $sender->sendMessage(ShopManager::TAG . "/상점설정 삭제 ( 상점이름 ) < 상점을 삭제합니다. >");
                    return true;
